...
- *本日 SmartNIC/DPU 議題已於 5/10–5/12 逐日浸透分析，包含 BlueField-3 通訊卸載量化研究、Blink 無 CPU 推理、SCENIC 串流計算 SmartNIC 、PITA 協議無關傳輸架構。請參考先前日期的 Network Daily Digest 以取得完整內容。*

## Ultra Ethernet Consortium & Standards

- *本週 UEC 議題已於 5/13 的 Network Daily Digest 中詳細分析（含 UEC 2.0 草案、MRC 協議、Tomahawk 6 與 EFA 2.0 對比）。請參考 5/13 日期的文章以取得完整內容。*

## Conference & Research Papers

以下為本週新發表、尚未在先前 digest 中出現過的論文：
...
3. **From Detection to Recovery: Operational Analysis on LLM Pre-training with 504 GPUs**（arXiv:2605.09370, May 10）
   ...
   - **實務價值**：強化了「網路必須具備自癒能力」的論點。

4. **Eliminating Hidden Serialization in Multi-Node Megakernel Communication**（arXiv:2605.00686, May 1）
